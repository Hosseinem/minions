...
using iterator_type = std::ranges::iterator_t<decltype(std::declval<seqan3::dna4_vector&>()
                                                       | opensyncmer_hash(seqan3::ungapped{4},
                                                                     3,6,
                                                                     seqan3::seed{0}))>;

static constexpr seqan3::shape ungapped_shape = seqan3::ungapped{4};
static constexpr seqan3::shape gapped_shape = 0b1001_shape;
static constexpr auto ungapped_view = opensyncmer_hash(ungapped_shape,
                                                  2,5,
                                                  seqan3::seed{0});
static constexpr auto ungapped_3_view = opensyncmer_hash(ungapped_shape,
                                            3,7,
                                            seqan3::seed{0});
static constexpr auto gapped_view = opensyncmer_hash(gapped_shape,
                                                2,6,
                                                seqan3::seed{0});
...
TYPED_TEST(opensyncmer_hash_view_properties_test, different_input_ranges)
{
...
TEST_F(opensyncmer_hash_test, ungapped)
{
    ...
    EXPECT_NO_THROW(text1 | opensyncmer_hash(ungapped_shape, 2,7));
    EXPECT_RANGE_EQ(result3_ungapped_3, text3 | ungapped_3_view);
    EXPECT_THROW((text3 | opensyncmer_hash(ungapped_shape, 1,5)), std::invalid_argument);
}

TEST_F(opensyncmer_hash_test, gapped)
{
    ...
    EXPECT_NO_THROW(text1 | opensyncmer_hash(gapped_shape, 2,4));
    EXPECT_THROW((text3 | opensyncmer_hash(gapped_shape, 1,6)), std::invalid_argument);
}
