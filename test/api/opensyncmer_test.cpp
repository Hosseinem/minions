...
inline static constexpr auto opensyncmer_view = opensyncmer(2,6);
...
struct iterator_fixture<iterator_type> : public ::testing::Test
{
    ...
    decltype(opensyncmer(seqan3::views::kmer_hash(text, seqan3::ungapped{4}), 2,6)) test_range =
    opensyncmer(vec, 2);
};
...
template <typename T>
class opensyncmer_view_properties_test: public ::testing::Test { };
...
TYPED_TEST_SUITE(opensyncmer_view_properties_test, underlying_range_types, );

class opensyncmer_test : public ::testing::Test
{
protected:
...
TYPED_TEST(opensyncmer_view_properties_test, concepts)
{
    ...
    auto v = text | kmer_view | opensyncmer_view;
    compare_types(v);
}

TYPED_TEST(opensyncmer_view_properties_test, different_inputs_kmer_hash)
{
    TypeParam text{'A'_dna4, 'C'_dna4, 'G'_dna4, 'T'_dna4, 'C'_dna4, 'G'_dna4, 'A'_dna4, 'C'_dna4, 'G'_dna4, 'T'_dna4,
                'T'_dna4, 'T'_dna4, 'A'_dna4, 'G'_dna4}; // ACGTCGACGTTTAG
    ...
    EXPECT_RANGE_EQ(ungapped, text | kmer_view | opensyncmer_view);
    EXPECT_RANGE_EQ(gapped, text | gapped_kmer_view | opensyncmer_view);
}

TEST_F(opensyncmer_test, ungapped_kmer_hash)
{
    EXPECT_RANGE_EQ(result1, text1 | kmer_view | opensyncmer_view);
    auto empty_view = too_short_text | kmer_view | opensyncmer_view;
    EXPECT_TRUE(std::ranges::empty(empty_view));
    EXPECT_RANGE_EQ(result3_ungapped, text3 | kmer_view | opensyncmer_view);

    auto v1 = text1 | kmer_view;
    EXPECT_RANGE_EQ(result1_distance, (seqan3::detail::opensyncmer_view<decltype(v1), true>(v1, 2,6)));
    auto v2 = text3 | kmer_view;
    EXPECT_RANGE_EQ(result3_distance, (seqan3::detail::opensyncmer_view<decltype(v2), true>(v2, 2,6)));
}

TEST_F(opensyncmer_test, gapped_kmer_hash)
{
    EXPECT_RANGE_EQ(result1, text1 | gapped_kmer_view | opensyncmer_view);
    auto empty_view = too_short_text | gapped_kmer_view | opensyncmer_view;
    EXPECT_TRUE(std::ranges::empty(empty_view));
    EXPECT_RANGE_EQ(result3_gapped, text3 | gapped_kmer_view | opensyncmer_view);
...
TEST_F(opensyncmer_test, combinability)
{
    auto stop_at_t = std::views::take_while([] (seqan3::dna4 const x) { return x != 'T'_dna4; });
    EXPECT_RANGE_EQ(result3_ungapped_stop, text3 | stop_at_t | kmer_view | opensyncmer_view);
    EXPECT_RANGE_EQ(result3_gapped_stop, text3 | stop_at_t | gapped_kmer_view | opensyncmer_view);

    auto v1 = text3 | stop_at_t | kmer_view;
    auto v2 = text3 | stop_at_t | kmer_view;
    EXPECT_RANGE_EQ(result3_distance_stop, (seqan3::detail::opensyncmer_view<decltype(v1), true>(v1, 2,6)));
    EXPECT_RANGE_EQ(result3_distance_stop, (seqan3::detail::opensyncmer_view<decltype(v2), true>(v2, 2,6)));

    auto start_at_a = std::views::drop(6);
    EXPECT_RANGE_EQ(result3_ungapped_start, (seqan3::detail::opensyncmer_view{text3 | start_at_a | kmer_view, 2,6}));

    auto v3 = text3 | start_at_a | kmer_view;
    auto v4 = text3 | start_at_a | gapped_kmer_view;
    EXPECT_RANGE_EQ(result3_distance_start, (seqan3::detail::opensyncmer_view<decltype(v3), true>(v3, 2,6)));
    EXPECT_RANGE_EQ(result3_distance_start, (seqan3::detail::opensyncmer_view<decltype(v4), true>(v4, 2,6)));
}
