...
struct syncmer_hash_fn
{
    ...
    */
                              ...
                              seed const seed = seqan3::seed{0x8F3F73B5CF1C9ADE}) const
    {
        ...
        auto kmers_forward_strand = std::forward<urng_t>(urange)
                                                 | seqan3::views::kmer_hash(seqan3::shape(seqan3::ungapped(kmers)))
                                                 | std::views::transform([seed] (uint64_t i)
                                                          {return i ^ seed.get();});

        auto kmers_reverse_strand = std::forward<urng_t>(urange)
        ...
        auto smers_forward_strand = std::forward<urng_t>(urange)
                                                 | seqan3::views::kmer_hash(seqan3::shape(seqan3::ungapped(smers)))
                                                 | std::views::transform([seed] (uint64_t i)
                                                          {return i ^ seed.get();});

        auto smers_reverse_strand = std::forward<urng_t>(urange)
                                                 | seqan3::views::complement
                                                 | std::views::reverse
                                                 | seqan3::views::kmer_hash(seqan3::shape(seqan3::ungapped(smers)))
                                                 | std::views::transform([seed] (uint64_t i)
        ...
        auto kmers_combined_strand = seqan3::views::zip(kmers_forward_strand, kmers_reverse_strand)
                                                 | std::views::transform([seed](std::tuple<uint64_t, uint64_t> i)
                                                          {return fnv_hash(std::get<0>(i) + std::get<1>(i), seed.get());});

        auto smers_combined_strand = seqan3::views::zip(smers_forward_strand, smers_reverse_strand)
                                                 | std::views::transform([seed](std::tuple<uint64_t, uint64_t> i)
                                                          {return fnv_hash(std::get<0>(i) + std::get<1>(i), seed.get());});

        return seqan3::detail::syncmer_view(smers_combined_strand, kmers_combined_strand, kmers - smers + 1);
    }
};
